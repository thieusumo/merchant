...
$(document).ready(function(){ 
  ...
  $('input.datepicker_expire').daterangepicker({
    ...
    function loadTicketBegin(){
      ...
      // var ticket_list_check = $("#ticket_list").val();
      // if(ticket_list_check == "")
        // ticket_list = Array();
      ...
      .done(function(data) {
        ...
        for(var i in data){
              ...
              if( ticket_no === data[i].booking_code) style_background = '#ffdf00';
    ...
    $('.save_ticket').click(function(e) {
        ...
        //IF CURRENT TICKET-> EDIT
        // if( number_of_ticket !== ""){
        //   ticket_list[number_of_ticket].time = time;
        //   ticket_list[number_of_ticket].date = date;
        //   ticket_list[number_of_ticket].order_list = order_list;
        //   ticket_list[number_of_ticket].customer_id = current_customer;

        //   // getPaymentCurrent(ticket_current);
        ...
        // }else{
          ...
          .done(function(data) {
            ...
            if(data == 0){
              ...
              //CREATE BILL NUMBER - BEGIN
             /* billOrder="";
              number_ticker = ticket_list.length + 1 ;
              numberOrder = (number_ticker.toString()).length;
              switch(numberOrder){

                case 1: billOrder = 'B000'+number_ticker; break;
                case 2: billOrder = 'B00'+number_ticker; break;
                case 3: billOrder = 'B0'+number_ticker; break;
                case 4: billOrder = 'B'+number_ticker; break;

                default : break;
              }*/
              // //CREATE BILL NUMBER - END
              //   var ticket = new Object();
              //   ticket.ticket_no =billOrder;
              //   ticket.time = time;
              //   ticket.date = date;
              //   ticket.customer_id=current_customer;
              //   ticket.order_list=order_list;
              //   ticket.ticket_parent = billOrder;
              //   ticket.ticket_combine = null;
              //   ticket.number = ticket_list.length;
              //   ticket.booking_id = booking_id;
              //   //AFTER DELETE UPDATE REASON FOR ticket.reason_delete
              //   ticket.reason_delete = "empty";
              //   //IF ticket.payment = 1 => PAYMENT SUCCESS
              //   ticket.payment = 0;
              //   ticket.balance_point = 0;
              //   ticket.point_cash = 0;
              //   ticket.coupon_code = null;
              //   ticket.coupon_amount =0;
              //   ticket.coupon_balance = 0;
              //   ticket.giftcard_code = null;
              //   ticket.giftcard_amount =0;
              //   ticket.giftcard_balance = 0;
              //   ticket.cash = 0;
              //   ticket.cash_back = 0;
              //   ticket.credit_number = null;
              //   ticket.credit_amount = 0;
              //   ticket.debit_number = null;
              //   ticket.debit_amount = 0;
              //   ticket.product_list = [];
              //   ticket.service_list = [];
              //   ticket.discount_amount = 0;
              //   ticket.giftcard_price = 0;
              //   ticket.total_point = 0;
              //   ticket.point_cash = 0;
              //   ticket.use_point = 0;
              //   ticket.use_amount = 0;
              //   ticket.tip = 0;
              //   ticket.staff_list_payment = [];
              //   ticket.tip_list = [];
              //   ticket.balance_amount_after_convert = 0;
              //   ticket.total_price = 0;
              //   ticket.discount_station = "";
              //   ticket.giftcard_pay = 0;
...
$(document).on('click','.reason-div',function(){
  // ticket_list.splice(number_of_ticket,1);
  ticket_current = "";
  $(".reason-voided-ticket-div").hide(500);
  $(".liststaffs_payment").show(300);
  var reason = $(this).attr('reason');
  //UPDATE TICKET IN TICKET LIST
  ticket_list[number_of_ticket].reason_delete = reason;
  //SAVE TICKET VOID TO DATABASE WITH REASON 
  saveTicketToDatabase(reason);
  // SAVE NEW TICKET LIST
  $.ajax({
    url: "{{route('set-session-payment')}}",
    type: "post",
...
//CLEAR TICKET WHEN DELETE
  function clearTicket(){
    ...
    //$(".time_payment_bottom").text(ticket_list[number_of_ticket].date+" "+ticket_list[number_of_ticket].time);
    $(".time_payment_middle").html("");
    $(".ticket_no").text("");
    $(".ticket_value").val("");
    //$(".total_price_bottom").text("$"+total_price);
    $(".sub_total").text("");
    $(".total_charge").text("");
    $(".balance_change").text("");
...
$("#edit_item").click(function(){
  ...
  if(correct_ticket == ""){
    ...
    order_list = service_list_payment;
    drawOrderList();
  }else
...
//SAVE TICKET TO DATABASE
  function saveTicketToDatabase(reason = ""){
    //SET BOOOKING_ID IN ARRAY
    var booking_id_list = [];
    if( reason === ""){
      for( var i in ticket_list ){
        if( ticket_list[i].ticket_parent == ticket_current && ticket_list[i].booking_id != 0 ){
          booking_id_list.push(ticket_list[i].booking_id);
        }
      }
    }
    $.ajax({
    ...
    .done(function(data) {
      ...
      if(data == 1){
        //DELETE TICKET
        var ticket_list_delete = [];
        for( var i in ticket_list ){
          if( ticket_list[i].ticket_parent == ticket_current ){
            ticket_list_delete.push(i);
          }
        }
        $.each(ticket_list_delete, function(index, val) {
           ticket_list[val].payment = 1;
        });
        //SAVE VALUE THIS TICKET FOR CHOOSE CORRECT TICKET
          //bối rối quá nên phải viết lại =.=! óc
            ticket_list[number_of_ticket].balance_point = ticket_list[number_of_ticket].amount_credit;
            ticket_list[number_of_ticket].point_cash = ticket_list[number_of_ticket].balance_point;
            ticket_list[number_of_ticket].coupon_code = ticket_list[number_of_ticket].coupon_code;
            ticket_list[number_of_ticket].coupon_amount =ticket_list[number_of_ticket].coupon;
            ticket_list[number_of_ticket].coupon_balance = ticket_list[number_of_ticket].coupon_balance;
            ticket_list[number_of_ticket].giftcard_code = ticket_list[number_of_ticket].giftcard_code;
            ticket_list[number_of_ticket].giftcard_amount =ticket_list[number_of_ticket].coupon_code;
            ticket_list[number_of_ticket].giftcard_balance = ticket_list[number_of_ticket].giftcard_balance;
            ticket_list[number_of_ticket].cash = ticket_list[number_of_ticket].cash;
            ticket_list[number_of_ticket].cash_back = ticket_list[number_of_ticket].cash_back;
            ticket_list[number_of_ticket].credit_number = ticket_list[number_of_ticket].card_number;
            ticket_list[number_of_ticket].credit_amount = ticket_list[number_of_ticket].amount_credit;
            ticket_list[number_of_ticket].debit_number = ticket_list[number_of_ticket].debit_number;
            ticket_list[number_of_ticket].debit_amount = ticket_list[number_of_ticket].amount_debit;
            ticket_list[number_of_ticket].product_list = product_list;
            ticket_list[number_of_ticket].service_list = service_list_payment;
            ticket_list[number_of_ticket].discount_amount = ticket_list[number_of_ticket].discount_amount;
            ticket_list[number_of_ticket].giftcard_price = ticket_list[number_of_ticket].giftcard_price;
            ticket_list[number_of_ticket].total_point = ticket_list[number_of_ticket].total_point;
            ticket_list[number_of_ticket].point_cash = ticket_list[number_of_ticket].point_cash;
            ticket_list[number_of_ticket].use_point = ticket_list[number_of_ticket].use_point;
            ticket_list[number_of_ticket].use_amount = ticket_list[number_of_ticket].use_amount;
            ticket_list[number_of_ticket].tip = ticket_list[number_of_ticket].tip;
            ticket_list[number_of_ticket].staff_list_payment = staff_list_payment;
            ticket_list[number_of_ticket].tip_list = tip_list;
            ticket_list[number_of_ticket].balance_amount_after_convert = ticket_list[number_of_ticket].balance_amount_after_convert;
            ticket_list[number_of_ticket].total_price = ticket_list[number_of_ticket].total_price;
            ticket_list[number_of_ticket].discount_station = discount_station;
            ticket_list[number_of_ticket].giftcard_pay = ticket_list[number_of_ticket].giftcard_pay;
        correct_ticket = "";
        loadTicketBegin();
        saveTicketListToSession();
        //RELOAD VIEW
...
//GET PAYMENT CURRENT
    $(document).on('click','.card-footer',function(e){
        ...
        if(correct_ticket == 1){
          getPaymentForCorrectTicket();
          loadCorrectTicketList();
        }
        else{
          drawPaymentList(number_of_ticket);
        }
        loadInfoCustomerFooter(number_of_ticket);
        getCustomer(number_of_ticket);
        loadTicketBegin();
    });
function loadInfoCustomerFooter(number_of_ticket,giftcode_point_earn = 0){
  // var customer_id = ticket_list[number_of_ticket].customer_id;
  var point_earn = 0;
...
function getCustomer(number_of_ticket){
  ...
  function getPaymentForCorrectTicket(){
        ...
        $("#cash").val("");
        //console.log(ticket_list);
        // $.each(ticket_list, function(index, val) {
        //     if(val['ticket_parent'] === ticket_no){
        //       //console.log(ticket_list);
        //         //console.log(val['order_list']);
                
        //         $.each(val['order_list'], function(index_orderlist, val_orderlist) {

        //             if($.inArray(val_orderlist['staff_name'],staff_list_payment) === -1){

        //                 staff_list_payment.push(val_orderlist['staff_name']);
        //             }
        //             service_price = val_orderlist['service_price'];

        //             if(val_orderlist['promotion_discount'] > 0){

        //             service_price = val_orderlist['service_price'] -
        //                   ( (val_orderlist['service_price'] * val_orderlist['promotion_discount'])/100 );
        //             }
        ...
        //         service_list_payment.push( val['order_list'] );
                
        //         //set current customer
                
        //     }
        //   });  
                $("#customer_id").val(ticket_list[number_of_ticket].customer_id);
                service_list_payment = ticket_list[number_of_ticket].service_list;
                product_list = ticket_list[number_of_ticket].product_list;
                staff_list_payment = ticket_list[number_of_ticket].staff_list_payment;
                tip_list = ticket_list[number_of_ticket].tip_list;
                discount_station = ticket_list[number_of_ticket].discount_station;

                payment_row = new Object();
                payment_row.booking_id = ticket_list[number_of_ticket].booking_id;
                payment_row.discount_amount = ticket_list[number_of_ticket].discount_amount;
                payment_row.cash = ticket_list[number_of_ticket].cash;
                payment_row.check = 0;
                payment_row.check_number = "";
                payment_row.giftcard_code = ticket_list[number_of_ticket].giftcard_code;
                payment_row.giftcard_pay = ticket_list[number_of_ticket].giftcard_pay;
                payment_row.card_number = ticket_list[number_of_ticket].credit_number;
                payment_row.amount_credit = ticket_list[number_of_ticket].credit_amount;
                payment_row.debit_number = ticket_list[number_of_ticket].debit_number;
                payment_row.amount_debit = ticket_list[number_of_ticket].debit_amount;
                payment_row.giftcard_price = ticket_list[number_of_ticket].giftcard_price;
                payment_row.giftcard_balance = ticket_list[number_of_ticket].giftcard_balance;
                payment_row.total_point = ticket_list[number_of_ticket].total_point;
                payment_row.balance_point = ticket_list[number_of_ticket].point_cash;
                payment_row.balance_amount_after_convert = ticket_list[number_of_ticket].balance_amount_after_convert;
                payment_row.point_cash = ticket_list[number_of_ticket].point_cash;
                payment_row.use_point = ticket_list[number_of_ticket].use_point;
                payment_row.use_amount = ticket_list[number_of_ticket].use_amount;
                payment_row.coupon_code = ticket_list[number_of_ticket].coupon_code;
                payment_row.coupon = ticket_list[number_of_ticket].coupon_amount;
                payment_row.coupon_balance = ticket_list[number_of_ticket].coupon_balance;
                payment_row.tip = ticket_list[number_of_ticket].tip;
                payment_row.time = ticket_list[number_of_ticket].time;
                payment_row.date = ticket_list[number_of_ticket].date;
                payment_row.total_price = ticket_list[number_of_ticket].total_price;
                payment_row.ticket_no = ticket_no;
                payment_row.giftcard_pay = ticket_list[number_of_ticket].giftcard_pay;
                payment_list.push(payment_row);

               service_list_payment = [].concat.apply([], service_list_payment);
  ...
  // function getPaymentCurrent(ticket_no){
  //       service_list_payment = [];
  //       tip_list = [];
  //       payment_list = [];
  //       staff_list_payment = [];
  ...
  //         });
  //               payment_row = new Object();
  //               payment_row.booking_id = booking_id;
  //               payment_row.discount_amount = 0;
  //               payment_row.cash = 0;
  //               payment_row.check = 0;
  //               payment_row.check_number = "";
  //               payment_row.giftcard_code = "";
  //               payment_row.giftcard_pay = 0;
  //               payment_row.card_number = "";
  //               payment_row.amount_credit = 0;
  //               payment_row.debit_number = "";
  //               payment_row.amount_debit = 0;
  //               payment_row.giftcard_price = 0;
  //               payment_row.giftcard_balance = 0;
  //               payment_row.total_point = 0;
  //               payment_row.balance_point = 0;
  //               payment_row.balance_amount_after_convert = 0;
  //               payment_row.point_cash = 0;
  //               payment_row.use_point = 0;
  //               payment_row.use_amount = 0;
  //               payment_row.coupon_code = "";
  //               payment_row.coupon = 0;
  //               payment_row.coupon_balance = 0;
  //               payment_row.tip = 0;
  //               payment_row.time = time;
  //               payment_row.date = date;
  //               payment_row.total_price = total_price;
  //               payment_row.ticket_no = ticket_no;
  //               payment_list.push(payment_row);
  ...
  //         drawPaymentList();
  // }
//END GET PAYMENT CURRENT

// function getPaymentCurrent(number_of_ticket){

// }

//DRAW CURRENT PAYMENT
    function drawPaymentList(number_of_ticket){
      // alert(number_of_ticket);
      ...
      console.log(ticket_list);
        ...
        //alert(ticket_list[number_of_ticket].total_point);
            ...
            // if(ticket_list[number_of_ticket].coupon != 0){
            if(ticket_list[number_of_ticket].coupon != 0){
                coupon_html = '<div style="float:left;width:30%;"></div><div style="float:left;width:40%;text-align:right" class="coupon_bill">Coupon</div><div style="float:right;width:30%;text-align:right;" class="ng-binding coupon_bill">'+parseFloat(ticket_list[number_of_ticket].coupon).toFixed(2)+'</div><div style="clear:both;display:block;"></div>';
            }
            // if(ticket_list[number_of_ticket].use_amount != 0){
            if(ticket_list[number_of_ticket].use_amount != 0){
                use_point_html = '<div style="float:left;width:30%;"></div><div style="float:left;width:40%;text-align:right" class="point">Use Point</div><div style="float:right;width:30%;text-align:right;" class="ng-binding point">'+parseFloat(ticket_list[number_of_ticket].use_amount).toFixed(2)+'</div><div style="clear:both;display:block;"></div>';
            }
            // if(ticket_list[number_of_ticket].balance_point != 0){
            if(ticket_list[number_of_ticket].balance_point != 0){
                total_point_html = '<div style="float:left;width:30%;"></div><div style="float:left;width:40%;text-align:right">Total Point</div><div style="float:right;width:30%;text-align:right;" class="ng-binding">'+parseFloat(ticket_list[number_of_ticket].use_amount).toFixed(2)+'</div><div style="clear:both;display:block;"></div>';
            ...
            //                     +parseFloat(ticket_list[number_of_ticket].coupon)
            ...
            total_payment = ( parseFloat(ticket_list[number_of_ticket].amount_credit)
                            +parseFloat(ticket_list[number_of_ticket].check)
                            +parseFloat(ticket_list[number_of_ticket].cash)
                            +parseFloat(ticket_list[number_of_ticket].amount_debit)
                            +parseFloat(ticket_list[number_of_ticket].coupon)
            ...
            cash_back = ( parseFloat(total_payment)
                         ...
                         +parseFloat(ticket_list[number_of_ticket].discount_amount) ).toFixed(2);

            total_charge = (parseFloat(total_price)-ticket_list[number_of_ticket].discount_amount).toFixed(2);
        ...
        $.each(service_list_payment, function(index, val) {
             service_list_html += '<div style="float:left;width:70%;">'+val['service_name']+'</div><div style="float:right;width:30%;text-align:right;" class="ng-binding">'+parseFloat(val['service_price']).toFixed(2)+'</div><div style="clear:both;display:block;"></div>';
             total_service_price +=  parseFloat(val['service_price']);
        });
            ...
            $.each(staff_list_payment, function(index_staff, staff_list_val) {
                ...
                $.each(service_list_payment, function(index, val) {
                  var service_price = 0;
                    ...
                    if(staff_list_val === val['staff_name']){

                        service_staff_list += '<div style="float:left;width:70%;">'+val['service_name']+'</div><div style="float:right;width:30%;text-align:right;" class="ng-binding">'+parseFloat(val['service_price']).toFixed(2)+'</div><div style="clear:both;display:block;"></div>';
                        service_price += parseInt( parseFloat(val['service_price']).toFixed(2) );
                    }
...
//ENTER AND CHANGE TIP INPUT
    $(document).on('keyup','#total_tip',function(){
        ...
        if(total_tip != ""){
            ...
            if(type_tip == 3){
                ...
                total_tip_change = ((parseFloat(total_tip)*parseFloat(total_price))/100).toFixed(2);
                    ...
                    $.each(staff_list_payment, function(index_staff, staff_list_val){
                        ...
                        $.each(service_list_payment, function(index, val) {
                            ...
                            {
                                service_price = val['service_price'] -
                                      ( (val['service_price'] * val['promotion_discount'])/100 );
                            }
                ...
                if(type_tip == 2){
                    ...
                    var service_amount = service_list_payment.length;
        ...
        $.each(payment_list, function(index, val) {
...
$(document).on('keyup','.tip_staff',function(){
  ...
  drawPaymentList();
  console.log(staff_list_payment);
  console.log(tip_list);
  console.log(ratio_tip);
}).delay(200);
...
$("#coupon_code").keyup(function(event) {
        ...
        .done(function(data) {
          console.log(data);
            ...
            if(ticket_list[number_of_ticket].total_charge >= parseFloat(data['coupon_cash']) ){
              ticket_list[number_of_ticket].coupon = data['coupon_cash'];
            ...
            console.info(service_list_payment);
            if(data['message'])
    ...
    $("#use_point").on('keyup',function(){
        ...
        if(user_point > ticket_list[number_of_ticket].balance_point){

            $(this).val(ticket_list[number_of_ticket].balance_point);
            user_point = ticket_list[number_of_ticket].balance_point;
        ...
        ticket_list[number_of_ticket].use_point = user_point;
        $(this).val(parseInt($(this).val()));

        if(user_point > 0 || user_point <= ticket_list[number_of_ticket].balance_point){
          $.ajax({
    ...
    $("#amount_credit").keyup(function(event) {
        ...
        ticket_list[number_of_ticket].amount_credit = amount_credit;
        }
        drawPaymentList();
    ...
    $("#card_number").keyup(function(event) {
        ...
        ticket_list[number_of_ticket].card_number = card_number;
    });
    $("#amount_debit").keyup(function(event) {

        var amount_debit = $(this).val();
        ...
        ticket_list[number_of_ticket].amount_debit = amount_debit;
        drawPaymentList();
        $(".debit_bill").addClass('yellow');
    });
    $("#debit_number").keyup(function(event) {

        var debit_number = $(this).val();
          ...
          if(window.confirm('Do you want split this ticket?') ){
            ...
            for (var i = 0; i < ticket_list.length; i++) {
              if(  ticket_list[i].ticket_no == ticket_current ){
                ...
                if(ticket_list[i].ticket_combine == "empty"){
                  toastr.error('Can not split this ticket!');
                }
                if(ticket_list[i].ticket_combine == null){
                  if(staff_list_payment.length == 1){
                    toastr.error('Can not split this ticket!');
                  }else{
                    var ticket_name = '';

                    for( var s in staff_list_payment){

                      var order_with_staff = [];

                      for( var l in ticket_list[i].order_list){

                        if( ticket_list[i].order_list[l].staff_name == staff_list_payment[s]){

                          order_with_staff.push(ticket_list[i].order_list[l]);
                        }
                      }
                      ticket_name += 'A';

                      var ticket = new Object();
                      ticket.ticket_no = ticket_list[i].ticket_no+sub_name_arr[s];
                      ticket.time = ticket_list[i].time;
                      ticket.date = ticket_list[i].date;
                      ticket.customer_id = ticket_list[i].customer_id;
                      ticket.order_list = order_with_staff;
                      ticket.ticket_parent = ticket_list[i].ticket_no+sub_name_arr[s];
                      ticket.ticket_combine = null;
                      ticket_list.push(ticket);
                    }
                    //SET TICKET CURRENT - DEFAULT NUMBET 0
                    ticket_current = ticket_list[i].ticket_no+sub_name_arr[0];

                    //DELETE CURRENT TICKET AFTER SPLIT WITH STAFF
                      ticket_list.splice(number_of_ticket,1);
                  }
                }
                // console.log(ticket_list);
                if(ticket_list[i].ticket_combine != null && ticket_list[i].ticket_combine != "empty"){

                  ticket_combine_array = ticket_list[i].ticket_combine;

                  for (var j = 0; j < ticket_list.length; j++){
                    if(jQuery.inArray(ticket_list[j].ticket_no, ticket_list[i].ticket_combine) !== -1 || ticket_list[j].ticket_no === ticket_current){
                      ticket_list[j].ticket_combine = null;
                      ticket_list[j].ticket_parent = ticket_list[j].ticket_no;
                    }
                  }
                }
              }

              if(ticket_combine_array.length !== 0){
                for( var k in ticket_combine_array){
                  if(ticket_list[i].ticket_no == ticket_combine_array[k]){
                    ticket_list[i].ticket_combine = null;
                    ticket_list[i].ticket_parent = ticket_list[i].ticket_no;
                  }
                }
              }
            }
          ...
          for( var i in ticket_list){
            if(ticket_list[i].ticket_no == ticket_current){
              if(ticket_list[i].ticket_combine != null && ticket_list[i].ticket_combine != ''){
                toastr.error('This ticket has combined with #'+ticket_list[i].ticket_combine+'. Split before combine with another!');
              }
              else{
                for(var i in ticket_list){
                  var stt = 0;
   ...
   $('#giftcard_gift_code').keyup(function(e){
    ...
    $(".add_giftcode").click(function(){
      // let gift_code = Math.random().toString(36).substring(7)+Math.random().toString(36).substring(7)+Math.random().toString(36).substring(7);
      // $("#giftcode_hidden_list").append(`<input type="hidden" id="`+gift_code+`" name="giftcode_code[]" value="`+gift_code+`" placeholder="">`);
  ...
  .always(function() {
    console.log("complete");
      ...
      ticket_list_html="";
        for(var i in ticket_list){
          order_list_in_ticket="";
            if(ticket_list[i].payment == 1)
            {
              var service_price = 0;
              let style_background = "";
              for(var j in ticket_list[i].order_list)
              {
                  order_list_in_ticket+='<p><b>'+ticket_list[i].order_list[j].staff_name+'</b><br>'+ticket_list[i].order_list[j].service_name+'</p>';
                  service_price += parseInt(ticket_list[i].order_list[j].service_price);
              }
              if( ticket_no === ticket_list[i].ticket_no) style_background = '#ffdf00';
              if( ticket_no !== ticket_list[i].ticket_no) style_background = '#959a9e';

              ticket_list_html+=`
              <div class="card ml-2 mb-1 " style="width: 12rem;" id="`+ticket_list[i].ticket_no+`">
                <div class="card-header card-footer" style="background-color:`+style_background+`"  ticket_no_footer="`+ticket_list[i].ticket_no+`" id="`+i+`">
                  <div class="float-left"><b>#`+ticket_list[i].ticket_no+ `</b></div>
                  <div class="float-right">`+ticket_list[i].time+`</div>
                </div>
                <div class="card-body card-footer" ticket_no_footer="`+ticket_list[i].ticket_no+`" id="`+i+`" style="height:5rem;overflow-y: auto">
                  `+order_list_in_ticket+`
                </div>
                <div class="col-md-12 text-danger text-right card-footer" style="position:absolute;bottom:0px;border:none"  ticket_no_footer="`+ticket_list[i].ticket_no+`" id="`+i+`">$`+service_price+`</div>
              </div>`;
            }
        }
        $('.correct_list').html(ticket_list_html);
        //console.log(ticket_list_html);
    }
// function loadInfoCustomerFooter(number_of_ticket,giftcode_point_earn = 0){
//   var customer_id = ticket_list[number_of_ticket].customer_id;
//   var point_earn = 0;
